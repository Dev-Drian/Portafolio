        ...
        <div className="my-6">
          {dataAboutPage.map((data) => (
            <div key={data.id} className="relate py-6 pl-8 sm:pl-32 group ">
              ...
              after:border-4 after:box-content after:border-slate-50 after:rounded-full sm:after:ml-[6.5rem]
              after:-translate-x-1/2 after:translate-y-1.5"
              >
                <time className="sm:absolute left-0 translate-y-0.5 inline-flex item-center" > {data.date}</time>
              </div>
            </div>
          ))}
        </div>
