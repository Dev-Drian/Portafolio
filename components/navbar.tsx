...
const Navbar = () => {
  ...
  return (
    <MotionTransition positions="right" className="fixed z-40 flex flex-col items-center justify-center w-full mt-auto h-max bottom-10">
      <nav className="flex items-center justify-center gap-1 px-4 py-2 rounded-full glass-strong border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
        {itemsNavbar.map((item) => (
          <motion.div
            ...
            className={`relative px-4 py-2.5 rounded-full cursor-pointer transition-all duration-300
              ${router === item.link 
